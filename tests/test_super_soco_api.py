...
async def test_api(hass, aioclient_mock):
    ...
    api = SuperSocoAPI(
        ...
        MOCK_CONFIG[CONF_PHONE_PREFIX],
        MOCK_CONFIG[CONF_PHONE_NUMBER],
        MOCK_CONFIG[CONF_PASSWORD],
    )

    # Login
    res_mock = json.loads(load_fixture("login.json"))

    aioclient_mock.post(f"{BASE_URL}/login", json=res_mock)
    assert await api.login() == res_mock

    # Get device
    res_mock = json.loads(load_fixture("device.json"))
    ...
    res_mock = json.loads(load_fixture("tracking_history.json"))
